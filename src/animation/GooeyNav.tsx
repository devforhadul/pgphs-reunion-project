...
const GooeyNav: React.FC<GooeyNavProps> = ({
  ...
  const createParticle = (
    ...
    const colorIndex = Math.floor(
      (noise(colors.length) + colors.length / 2) % colors.length
    );
    return {
      start: getXY(d[0], particleCount - i, particleCount, noise),
      end: getXY(d[1] + noise(7), particleCount - i, particleCount, noise),
      time: t,
      scale: 1 + noise(0.2),
  ...
  const makeParticles = (element: HTMLElement) => {
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      const p = createParticle(i, t, d, r, noise);
